...
namespace OTL
{
    ...
    {
        VectorXd h1(this->reservoir_size);

        // update function
        h1 = this->W * this->xn + this->W_in * input;

        // use the activation function
        VectorXd xn;
        this->activation(h1, xn);

        // use the leak rate if needed

        if (this->leak_rate > 0.0)
        {
            xn = (1.0 - this->leak_rate) * xn + this->leak_rate * this->xn;
        }

        // concated state vector
        if (this->use_inputs_in_state)
        {
            this->h2.segment(0, this->reservoir_size) = xn;
            this->h2.segment(this->reservoir_size, this->input_dim) = input;
        }
        else
        {
            this->h2 = xn;
    ...
    void Reservoir::update(const VectorXd &input, const VectorXd &output_feedback)
    {
        ...
        // update function
        state_before_activation = this->W * this->xn +
                                  this->W_in * input +
        ...
        if (this->leak_rate > 0.0)
        {
            new_state = (1.0 - this->leak_rate) * new_state +
                        this->leak_rate * this->xn;
        }

        // construct augmented state vector
        if (this->use_inputs_in_state)
        {
            this->h2.segment(0, this->reservoir_size) =
                h2;
            this->h2.segment(this->reservoir_size, this->input_dim) =
                input;
        }
        else
        {
    ...
    {
        this->h2 = VectorXd::Zero(this->state_size);
    }
    ...
    **/
    void Reservoir::save(std::string filename)
    {
        ...
        saveMatrixToStream(out, this->W_in);
        saveMatrixToStream(out, this->output_feedback_weights);
        saveMatrixToStream(out, this->W);
        saveVectorToStream(out, this->xn);
        saveVectorToStream(out, this->h2);
    ...
    **/
    void Reservoir::load(std::string filename)
    {
        ...
        readMatrixFromStream(in, this->W_in);
        readMatrixFromStream(in, this->output_feedback_weights);
        readMatrixFromStream(in, this->W);
        readVectorFromStream(in, this->xn);
        readVectorFromStream(in, this->h2);
        ...
        unsigned int random_seed)
    {
        ...
        this->W_in = MatrixXd::Ones(reservoir_size, input_dim) * this->input_weight;
        ...
        this->W = MatrixXd::Random(reservoir_size, reservoir_size);
    ...
    double Reservoir::getActualSpectralRadius(void)
    {
        ...
        Eigen::SelfAdjointEigenSolver<MatrixXd> eigen_solver(this->W);
    ...
    void Reservoir::setInputWeights(MatrixXd &input_weights)
    {
        ...
        if (input_weights.cols() != this->W_in.cols())
        {
            throw OTLException("Columns do not match");
        }
        if (input_weights.rows() != this->W_in.rows())
        {
            throw OTLException("Rows do not match");
        }

        this->W_in = input_weights;
    }
    ...
    void Reservoir::setReservoirWeights(MatrixXd &reservoir_weights)
    {
        ...
        if (reservoir_weights.cols() != this->W.cols())
        {
            throw OTLException("Columns do not match");
        }
        if (reservoir_weights.rows() != this->W.rows())
        {
            throw OTLException("Rows do not match");
        }

        this->W = reservoir_weights;
    }
